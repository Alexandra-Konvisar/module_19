...
def test_add_new_pets_photo_txt(name=pets_data.name_latin, animal_type= pets_data.animal_type_latin, age= pets_data.age_norm,
    ...
    """Проверяем добавление нового питомца, входные данные имя латиница, тип питомца латиница, возраст 2, фото расширение txt - все данные тип строка"""
...
def test_clear_list_pest():
    """Очищаем список от созданных питомцев после тестов(пока так, позже нужно переделать на удаление после создания"""
    _, auth_key = pf.get_api_key(data.valid_email, data.valid_password)
    while True:
